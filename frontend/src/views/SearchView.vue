    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5 mb-6">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        <div>
          ...
          <input v-model="filters.location" class="field-input" placeholder="City, Country" />
        </div>
        <div>
          <label class="field-label">Min Age</label>
          <input v-model.number="filters.ageMin" type="number" min="18" class="field-input" />
        </div>
        <div>
          <label class="field-label">Max Age</label>
          <input v-model.number="filters.ageMax" type="number" max="120" class="field-input" />
        </div>
        <div>
          <label class="field-label">Keyword</label>
          <input v-model="filters.keyword" class="field-input" placeholder="Name, bio..." />
        </div>
      </div>

      <div class="flex gap-4 items-end">
        <div class="flex-1">
          <label class="field-label">Interests (any match)</label>
          <input v-model="interestInput" class="field-input" placeholder="e.g. hiking, music" @keydown.enter.prevent="addInterest" />
        </div>
        <button type="button" @click="addInterest" class="px-4 py-2 bg-pink-500 text-white rounded-lg text-sm hover:bg-pink-600">Add</button>
      </div>
      ...
      <div v-if="filters.interests.length" class="flex flex-wrap gap-2 mt-3">
        <span v-for="i in filters.interests" :key="i"
          ...
          {{ i }}
          <button @click="removeInterest(i)" class="font-bold hover:text-red-500">×</button>
        </span>
      </div>
