  ...
  <div class="min-h-screen bg-linear-to-br from-pink-200 to-blue-200 dark:from-blue-950 dark:to-pink-950">
    <div class="max-w-5xl mx-auto px-4 py-8">
      ...
      <div v-else-if="profiles.length === 0" class="text-center py-20">
        ...
        <router-link to="/browse" class="mt-4 inline-block px-6 py-3 text-base bg-pink-500 text-white rounded-full hover:bg-pink-600">
          Browse People
        </router-link>
      </div>
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div class="p-3">
            ...
            <p class="text-xs text-gray-400 mb-2 truncate">{{ item.profile?.location || '' }}</p>
            <div class="flex flex-col sm:flex-row gap-2">
              <router-link
                v-if="item.profile"
                :to="`/profile/${item.profile.id}`"
                class="flex-1 text-center text-sm py-2 border border-pink-300 text-pink-500 rounded-full hover:bg-pink-50 dark:hover:bg-pink-900/20"
              >
                Profile
              </router-link>
              <router-link
                :to="`/messages/${item.otherUserId}`"
                class="flex-1 text-center text-sm py-2 bg-pink-500 text-white rounded-full hover:bg-pink-600"
              >
                Chat
              </router-link>
            </div>
