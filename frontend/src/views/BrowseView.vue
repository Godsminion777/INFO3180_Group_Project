  ...
  <div class="min-h-screen bg-linear-to-br from-pink-200 to-blue-200 dark:from-blue-950 dark:to-pink-950">
    <div class="max-w-5xl mx-auto px-4 py-8">
      <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h1 class="text-2xl font-bold text-blue-900 dark:text-pink-400">Discover People</h1>
        <div class="flex items-center gap-3">
          <select v-model="sortBy" class="text-base border border-blue-300 dark:border-pink-900 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 focus:outline-none">
            <option value="score">Best Match</option>
            <option value="newest">Newest</option>
            <option value="age">By Age</option>
          </select>
          <button @click="showFilters = !showFilters" class="flex items-center gap-1 text-base px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">
            Filters
          </button>
        </div>
      </div>
      ...
      <!-- Filters panel -->
      <div v-if="showFilters" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          <label for="filter-age-min" class="text-xs font-medium text-gray-600 dark:text-gray-400">Min Age</label>
          <input id="filter-age-min" v-model.number="filters.ageMin" type="number" min="18" max="100" class="filter-input" />
        </div>
        <div>
          <label for="filter-age-max" class="text-xs font-medium text-gray-600 dark:text-gray-400">Max Age</label>
          <input id="filter-age-max" v-model.number="filters.ageMax" type="number" min="18" max="100" class="filter-input" />
        </div>
        <div>
          <label for="filter-location" class="text-xs font-medium text-gray-600 dark:text-gray-400">Location</label>
          <input id="filter-location" v-model="filters.location" type="text" autocomplete="address-level2" placeholder="City..." class="filter-input" />
        </div>
        <div>
          <label for="filter-interest" class="text-xs font-medium text-gray-600 dark:text-gray-400">Interest</label>
          <input id="filter-interest" v-model="filters.interest" type="text" placeholder="e.g. hiking" class="filter-input" />
        </div>
      </div>
      ...
      <div v-if="currentCandidate" class="flex flex-col items-center mb-10">
        ...
        <div class="flex items-center gap-6 mt-6">
          <button @click="doAct('dislike')" aria-label="Dislike" class="w-16 h-16 bg-white dark:bg-gray-700 rounded-full shadow-md flex items-center justify-center text-2xl hover:scale-110 transition-transform border border-gray-100 dark:border-gray-600">
            👎
          </button>
          <button @click="doAct('superlike')" aria-label="Superlike" class="w-14 h-14 bg-blue-50 dark:bg-blue-900/30 rounded-full shadow-md flex items-center justify-center text-xl hover:scale-110 transition-transform border border-blue-100 dark:border-blue-700">
            ⭐
          </button>
          <button @click="doAct('like')" aria-label="Like" class="w-16 h-16 bg-pink-500 rounded-full shadow-md flex items-center justify-center text-2xl hover:scale-110 transition-transform">
            ❤️
          </button>
        </div>
      </div>
...
.filter-input {
  @apply w-full mt-1 px-3 py-2 text-base border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:outline-none focus:ring-1 focus:ring-pink-300;
}
</style>